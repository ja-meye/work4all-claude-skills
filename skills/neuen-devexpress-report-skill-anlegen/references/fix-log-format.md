...
- Pro **Skill-ID + Version-Kombination** enthält der Block höchstens eine maßgebliche Zeile — die der jeweils letzten **abgeschlossenen** Anwendung dieser exakten Kombination (was "abgeschlossen" heißt, siehe Regel 7 unten; wie eine erneute Anwendung derselben Kombination mit einer bereits vorhandenen Zeile umgeht, regelt Regel 1). Unterschiedliche Kombinationen (derselbe Skill in einer anderen Version, oder ein anderer Skill) stehen als eigene Zeilen **zwischen** der Kopf- und der Fußzeile, in der Reihenfolge ihres ersten Auftretens (älteste Kombination zuerst).
...
1. **Eine Zeile pro Skill-ID+Version — ersetzen statt anhängen bei identischer Kombination (korrigiert 16.09.2026, siehe Hinweis unten).** Vor dem Schreiben einer neuen Zeile durchsucht ein Skill den bestehenden Log-Block nach einer Zeile mit EXAKT derselben Skill-ID **und** EXAKT derselben Version (reiner String-Vergleich der Versionsangabe, kein „≥"):
   - **Gefunden (identische Skill-ID + identische Version):** Diese bestehende Zeile wird durch die neue ersetzt (neuer Zeitstempel, neues `<Ergebnis>`, ggf. neues `<Übersprungen>`) — an ihrer bisherigen Position im Block. Sie wird nicht ans Ende verschoben, und es wird keine zusätzliche Zeile angehängt. Grund: Mehrere Durchläufe **derselben** Skill-Version (typischerweise mehrere Korrekturrunden am selben Bearbeitungstag, bis ein Fix vollständig und fehlerfrei sitzt) sind für den Leser keine dauerhaft relevanten Einzelereignisse — nur der jeweils letzte, tatsächlich gültige Stand dieser Kombination zählt.
   - **Nicht gefunden (ein anderer Skill, ODER derselbe Skill in einer anderen Version):** Es wird eine neue Zeile ergänzt, immer direkt vor der Fußzeile eingefügt. Zeilen anderer Skill-ID+Version-Kombinationen sind davon nie betroffen — sie werden durch das Ersetzen nie entfernt, verschoben oder inhaltlich verändert.

   **Beispiel — drei Korrekturrunden derselben Version am selben Tag:** Nach der ersten Runde steht im Block genau eine Zeile:
   ```
   // === work4all-log (v3) ===
   // DXJ0001 | v1.10.1 | 2026-09-09T16:34:23 | fix-folgeseiten-uebertrag-problem | geändert
   // === end work4all-log ===
   ```
   Runde 2 (17:05:57) und Runde 3 (17:30:50) derselben Version `v1.10.1` **ersetzen** diese eine Zeile jeweils erneut — am Ende steht weiterhin nur eine Zeile, mit dem Stand der letzten Runde:
   ```
   // === work4all-log (v3) ===
   // DXJ0001 | v1.10.1 | 2026-09-09T17:30:50 | fix-folgeseiten-uebertrag-problem | geändert
   // === end work4all-log ===
   ```
   Wird DXJ0001 später tatsächlich in einer neuen Version (`v1.10.2`) angewendet, ist das eine ANDERE Kombination und bekommt eine eigene, zusätzliche Zeile — die `v1.10.1`-Zeile bleibt unverändert stehen:
   ```
   // === work4all-log (v3) ===
   // DXJ0001 | v1.10.1 | 2026-09-09T17:30:50 | fix-folgeseiten-uebertrag-problem | geändert
   // DXJ0001 | v1.10.2 | 2026-09-10T08:30:50 | fix-folgeseiten-uebertrag-problem | geändert
   // === end work4all-log ===
   ```
   **Hinweis zur Historie dieser Regel:** Vor dieser Klarstellung (16.09.2026) sagte diese Regel wörtlich „Append-only … niemals löschen, verändern oder überschreiben", was in der Praxis zu mehreren fast identischen Zeilen für dieselbe Skill-Version führte (drei Korrekturrunden = drei Zeilen). Das entsprach nicht dem beabsichtigten Verhalten und wurde einmalig manuell auf eine Beispieldatei (`dxAio_template`, Lauf 09.09.2026) korrigiert, bevor die Spezifikation hier nachgezogen wurde. **Was unverändert gilt:** Zeilen ANDERER Skill-ID+Version-Kombinationen werden nach wie vor niemals gelöscht, verändert oder überschrieben — nur das Verhalten bei einer exakt identischen Kombination wurde korrigiert.
2. **Vor Anwendung eines Fixes: Idempotenz-Check — nur relevant bei `geändert`.** Bevor ein Verbesserungs-Skill einen Fix anwendet, prüft er den bestehenden Log-Block: Ist die eigene Skill-ID bereits mit dem Ergebnis `geändert` und einer Version ≥ der aktuellen Skill-Version vorhanden? Falls ja, wird der Fix **nicht blind erneut angewendet** — stattdessen meldet Claude dies dem Nutzer ("Skill DXJ0001 v1.0.0 hat diese Datei laut Log bereits am ... geändert — soll ich trotzdem erneut prüfen/anwenden?") und wartet auf Bestätigung. Findet sich stattdessen nur eine frühere Zeile mit `keine Änderung nötig` oder `abgebrochen: ...`, greift der Idempotenz-Block **nicht** — es gibt in diesem Fall nichts, was "erneut angewendet" würde, also läuft die Diagnose ganz normal neu (die Datei könnte sich seither geändert haben, z.B. durch einen Designer-Speichervorgang). Bestätigt der Nutzer trotz Idempotenz-Warnung eine erneute Anwendung derselben Skill-ID+Version, greift für die resultierende neue Zeile ganz normal Regel 1: sie ersetzt die bestehende Zeile dieser Kombination, statt eine weitere anzuhängen. Da pro Skill-ID+Version seit Regel 1 höchstens eine Zeile existiert, genügt für diesen Check ein Blick auf die (höchstens eine) Zeile der eigenen Skill-ID mit einer Version ≥ der aktuellen — mehrere Zeilen derselben Kombination, über die hinweg gesucht werden müsste, kann es nicht mehr geben.
