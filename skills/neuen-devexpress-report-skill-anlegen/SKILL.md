...
Vollständige Spezifikation: `references/fix-log-format.md`. Kurzfassung: jeder Skill bekommt eine Skill-ID (`references/skill-id-registry.md`), und jede Anwendung eines Verbesserungs-Skills auf eine `.repx` hinterlässt einen Log-Eintrag ganz oben im eingebetteten C#-Skript der Datei — pro Skill-ID+Version-Kombination genau eine maßgebliche Zeile (mehrere Durchläufe derselben Version ersetzen diese eine Zeile; eine andere Version oder ein anderer Skill bekommt eine eigene, zusätzliche Zeile).
...
- v0.8.1 — Regelkorrektur in `references/fix-log-format.md` Regel 1 (Nutzerwunsch, 16.09.2026, Report `dxAio_template`, M001): Die Regel sagte wörtlich „Append-only … niemals löschen, verändern oder überschreiben", was in der Praxis zu mehreren fast identischen Log-Zeilen für dieselbe Skill-ID+Version führte, wenn ein Fix erst nach mehreren Korrekturrunden am selben Tag fehlerfrei durchlief (konkret beobachtet: `DXJ0001 v1.10.1` dreimal im selben Block, nur mit unterschiedlichem Zeitstempel). Das entsprach nicht dem beabsichtigten Verhalten. Neue Fassung von Regel 1: Pro Skill-ID+Version-Kombination existiert höchstens eine maßgebliche Zeile — eine erneute Anwendung derselben exakten Kombination **ersetzt** diese eine Zeile (Zeitstempel/Ergebnis/Übersprungen), statt eine weitere anzuhängen; nur eine ANDERE Kombination (anderer Skill, oder derselbe Skill in einer anderen Version) bekommt weiterhin eine neue, zusätzliche Zeile. Zeilen anderer Kombinationen bleiben davon unberührt und werden nie entfernt oder verändert. Regel 2 (Idempotenz-Check) entsprechend um einen klarstellenden Satz ergänzt (pro Kombination jetzt höchstens eine Kandidatenzeile). Baustein 7 (Kurzfassung) und `fix-folgeseiten-uebertrag-problem/scripts/validate_repx.py` Check `C19` (verglich Log-Zeilen bisher als exakte Strings und hätte ein korrektes Ersetzen fälschlich als Verlust gemeldet, prüft jetzt nur noch auf verlorene Skill-ID+Version-Kombinationen) im selben Zug mitgezogen — Details und Testfälle: `fix-folgeseiten-uebertrag-problem/references/skill-changelog.md` v1.12.1.
