...
**Referenzbeispiel:** `assets/dxAio_template_REFERENZ_KORRIGIERT.repx` ist der vollständig durchgefixte, vom Kunden in DevExpress getestete und bestätigte Report, an dem diese gesamte Methodik entwickelt wurde — ein Proof-of-Concept mit allen oben genannten Fixes bereits angewendet (inkl. beider `KeepTogether`-Ebenen). Bei einer neuen Report-Variante lohnt sich ein struktureller Vergleich (Bandnamen, Scripts-Verdrahtung, Sichtbarkeits-Handler) gegen diese Referenz, um schnell zu sehen, welche der bekannten Muster bereits vorhanden/bereits gefixt sind und welche noch fehlen.
