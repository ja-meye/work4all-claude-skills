...
2. Mindesthöhe von `tb_ÜbertragOben` (und dem umgebenden `SubBand`, z. B. `Sub_UebertragOben`) auf **70** anheben — dabei **nur anheben, nie verringern**: ist bereits ein höherer Wert eingestellt, diesen unverändert lassen.

   > **Verhältnis zu Muster (i), verbindlich seit 04.09.2026 — sonst widersprechen sich die beiden Muster:** Wird an derselben Stelle zusätzlich Muster (i) angewendet (bandeigenes `BeforePrint` mit `e.Cancel` gegen den Seite-1-Leerraum), dann **wandert die 70 vom Design in die Laufzeit**: der Design-Wert wird bewusst auf Mindestmaß abgesenkt (Muster (i), Punkt 4) und die echten 70 werden in einem eigenen, tabelleneigenen `BeforePrint` (`tb_ÜbertragOben_BeforePrint`, analog zum bereits bestehenden `tb_Überschriften_Folgeseite_BeforePrint`) wiederhergestellt (Muster (i), Punkt 3, Bedingung über `pageCounter`: `tbl.HeightF = (pageCounter == 1) ? 1f : 70f;`). Das „nie verringern" aus diesem Punkt gilt dann nur noch für den **wirksamen** Wert zur Druckzeit, nicht für den Design-Wert. Reihenfolge deshalb immer: erst prüfen, ob (i) zutrifft, dann (g) entsprechend als Laufzeitwert umsetzen — nicht beide Muster unabhängig voneinander auf denselben Design-Wert anwenden.
   >
   > **Aktualisiert 08.09.2026 (`known-issues.md` Eintrag 35):** Der ursprünglich am 28.08. bestätigte Laufzeitwert **50** war laut Testdruck des Kunden zu niedrig bemessen und ist überholt. Aktuell bestätigt und verbindlich für `dxAio_template`: **70**. Wie schon bei der Padding-Top-Korrektur in Eintrag 15/34 gilt: bei einem Widerspruch zwischen einem älteren, hier dokumentierten Zahlenwert und dem tatsächlich in einer aktuell bestätigten Datei stehenden Wert entscheidet die Datei, nicht die Dokumentation — diese Zeile wird bei der nächsten Korrektur ebenso wieder aktualisiert.
...
**Automatisierungssicherheit:**
- Punkt 1 (KeepTogether zurücknehmen) und Punkt 2 (Mindesthöhe 70): **Automatisch sicher, sobald eine bestätigte Referenzdatei vorliegt und denselben Zustand zeigt** (Grundmuster seit 28.08. bestätigt, Laufzeitwert seit 08.09. bei 70) — dann NICHT mehr als Vorschlag mit Rückfrage behandeln, sondern direkt umsetzen, ohne erneut nachzufragen. Nur wenn keine Referenz vorliegt oder sie dieses Muster nicht eindeutig bestätigt: *Vorschlag mit Rückfrage*.
...
4. Die eigenen, separat persistierten Standardhöhen (`SizeF`/`HeightF`, ggf. im `<Localization>`-Block, siehe Ursache 4 und `known-issues.md` Eintrag 6) aller betroffenen verschachtelten Controls UND des äußeren Bands selbst auf einen kleinen Wert absenken. **Wichtig:** DevExpress erzwingt für `HeightF`/`H` einen Mindestwert von **5** — ein im Code gesetzter Wert unter 5 (z. B. `1`) wird beim Lesen/Anzeigen im Enduserdesigner-Properties-Panel stillschweigend auf 5 angehoben; direkt den erzwungenen Minimalwert 5 verwenden vermeidet eine unnötige, in der Praxis sichtbare Restunschärfe. **Praxiswert aus dem bestätigten Stand (04.09.2026):** dort stehen die verschachtelten Controls und die reinen Rechen-Bänder auf **1**, das sichtbare Anzeige-Band auf **5**. Beides ist in Ordnung — ein vorhandener Wert `1` wird nicht „korrigiert", er zeigt sich im Properties-Panel lediglich als 5.
   ...
   > **Verbindliche Ergänzung seit 10.09.2026 (`known-issues.md` Eintrag 39) — „alle betroffenen verschachtelten Controls" ist wörtlich zu nehmen, nicht nur die neu angelegten:** Bei einem Band-Split (Teil 1) werden zwangsläufig ein oder mehrere neue Helper-Controls angelegt — deren Höhe wird dabei erfahrungsgemäß zuverlässig gesetzt, weil sie im selben Arbeitsschritt entstehen. Das äußere Anzeige-Band enthält daneben aber häufig zusätzlich bereits **vorher vorhandene** Geschwister-Controls im selben `<Controls>`-Element (z. B. weitere Berechnungs-/Kontroll-Labels, die schon vor dem Split dort standen) — diese werden beim Fokus auf die neu angelegten Elemente leicht übersehen, tragen aber genauso eine eigene, unabhängig persistierte Höhe (Ursache 3) und müssen ebenso auf den Minimalwert abgesenkt werden. **Vorgehen:** Vor Abschluss von Punkt 4 IMMER die vollständige `<Controls>`-Sammlung des Anzeige-Bands durchgehen (nicht nur die in Punkt 1 neu erzeugten Elemente) und für JEDES Kind-Control einzeln die wirksame Höhe prüfen (direktes Attribut UND `<Localization>`-Eintrag, siehe Ursache 4) — als sichtbare Liste im Befund an den Nutzer (Vollständigkeits-Heuristik, SKILL.md Schritt 2). **Konkreter, vom Kunden bestätigter Fall (M001, `dxAio_template`, Band `Sub_POS_Ueberschriften_Folgeseiten`):** Ein Fix-Lauf hatte nur das neu angelegte `lblDocGesamtGesPreis` ordnungsgemäß behandelt; zwei bereits vorher vorhandene Geschwister-Controls im selben Band (`xrLabel45` mit `SizeF="100,90"`, `lblCarryHelperOben` mit `SizeF="160,40"`, beide über einen `<LocalizationItems>`-Eintrag persistiert) blieben unangetastet und erzeugten beim Testdruck zusätzlichen, sichtbaren Abstand. Der Kunde hat alle drei Controls anschließend manuell in Visual Studio auf Höhe 5 korrigiert.
