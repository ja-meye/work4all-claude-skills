...
Nicht jede im DevExpress-Designer manuell geänderte Eigenschaft landet als direktes XML-Attribut auf dem betroffenen Element. Insbesondere `HeightF`, `SizeF`, `LocationFloat` und — seit 04.09.2026 konkret bestätigt (siehe unten) — auch `Visible` werden häufig stattdessen indirekt in einem separaten Container-Element weiter oben in der Datei abgelegt, als Liste von Einträgen der Form:

```xml
<Item354 Ref="2991" Component="#Ref-1931" Culture="Default" Path="HeightF" Data="40" />
```

**Der tatsächliche Container-Tag-Name ist `<LocalizationItems>`** (die einzelnen Einträge darin heißen `<Item0>`, `<Item1>`, ... `<ItemN>`) — „`<Localization>`-Block" ist der in dieser Skill-Familie eingebürgerte Kurzname für diesen Mechanismus, nicht der wörtliche XML-Tag. Bei einer Stringsuche im Dateiinhalt nach `LocalizationItems` suchen, nicht nach `<Localization`.
...
**Bestätigter Fall für `Path="Visible"` (Report `dxAio_template`, Sektion `Sub_Adresse`, 04.09.2026):** Ein Platzhalter-Label sollte unsichtbar gemacht werden. Das direkte Attribut `Visible="false"` wurde korrekt gesetzt, im Designer-Properties-Panel erschien das Element aber weiterhin als `Visible = True` — weil ein bereits vorhandener `<LocalizationItems>`-Eintrag (`Path="Visible"`, `Data="true"`) für den `Ref` dieses Elements das direkte Attribut übersteuerte. Erst nach Änderung BEIDER Stellen (direktes Attribut UND `LocalizationItems`-Eintrag) zeigte der Designer den korrekten Wert. Siehe `known-issues.md` Eintrag 30 und `fix-catalog.md` Muster (j) für den vollständigen Fix. Nicht zu verwechseln mit Eintrag 26 (`ExpressionBinding` überschreibt `Visible` erst zur Laufzeit) — beide Mechanismen sind unabhängig voneinander zu prüfen.

**Praktische Konsequenz:** Bei jeder Eigenschafts-Änderung, die per Diff gegen eine im Designer bearbeitete Datei nachvollzogen werden soll — nicht nur bei Höhe/Größe/Position, sondern grundsätzlich bei JEDER Property, `Visible` eingeschlossen:
...
4. Wird ein Element komplett aus dem Report entfernt (nicht nur unsichtbar gemacht): alle seine eigenen `<LocalizationItems>`-Einträge (per `Component="#Ref-<RefDesElements>"` identifiziert) im selben Arbeitsschritt mitentfernen — sonst bleiben verwaiste Einträge zurück, die auf einen nicht mehr existierenden `Ref` zeigen. Anschließend die betroffene `ItemN`-Sammlung lückenlos neu nummerieren (siehe `known-issues.md` Eintrag 22).

Ein reiner Element-für-Element-Vergleich der vermuteten Stelle kann diese Änderungen komplett übersehen (weil weder vorher noch nachher ein direktes Attribut existiert) — zuverlässiger ist ein vollständiger, positionsweiser Baumvergleich der GESAMTEN Datei (alle Elemente in Dokumentreihenfolge, `Ref`-Werte beim Vergleich ignorieren), der solche indirekten Änderungen automatisch mit erfasst. Details und ein konkretes Beispiel dazu in `known-issues.md`, Einträge 6 und 7 (dort auch: nicht jede so gefundene Verschiebung ist eine echte Änderung — manche sind harmlose, rein designtime-relevante Umsortierungen). Zur robusten Vergleichsmethodik bei mehreren, zwischenzeitlich vom Designer resavten Dateiversionen (Ref-Neunummerierung bei jedem Speichern) siehe `known-issues.md` Einträge 29–31 und `fix-catalog.md` Muster (j).
