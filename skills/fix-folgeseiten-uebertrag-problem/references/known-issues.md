...
- 36. Eine Katalog-Formulierung im Plural („die/der unmittelbar nachfolgende(n) SubBand(s)") wurde nur am ersten, symptomauslösenden Fund behoben, nicht an allen strukturell analogen Stellen
- 37. Ein neu angelegtes Sammlungs-Element mit ungültigem Platzhalter-Tag-Namen (`<ItemPLACEHOLDER>` statt `<ItemN>`) ergibt gültiges XML, wird aber von DevExpress beim Laden stillschweigend verworfen — und bei einem späteren Rebuild derselben Sammlung endgültig gelöscht
- 38. `validate_repx.py` Check `C07` meldet bei isoliertem Aufruf ohne `--baseline` Falsch-Positive für Methoden, die ausschließlich in einem Subreport-eigenen `ScriptsSource`-Block definiert sind (Namensraum-Verwechslung zwischen Haupt- und Subreport-Skript)
- 39. Muster (i) Punkt 4 wurde nur auf die neu angelegten Helper-Controls angewendet, nicht auf bereits vorher vorhandene Geschwister-Controls im selben Band
...
**Wann aufgefallen:** 09.09.2026, Report `dxAio_template` (M001), Nachbesserung 2 — Auslöser war ein Designer-Screenshot des Kunden („Padding nicht oben richtig gesetzt bei diversen Feldern").

**Was passiert ist:** Muster (h) Punkt 2 (`DXJ0001.E`) verlangt laut Katalog, nach Behebung von Punkt 1 (`AllowMarkupText` entfernen) zusätzlich das Top-Padding „der/den unmittelbar nachfolgende(n) Zeile(n)/SubBand(s)" moderat zu erhöhen — ausdrücklich im Plural formuliert und bereits mit vier konkreten Zellnamen in Eintrag 15 bestätigt (`Sub_Rabatt`, `Sub_Rabatt2`, `Sub_LieferterminPOS`, `Sub_ZollinformationenPOS`). Der vorangegangene Lauf (Nachbesserung 1) hatte den Diagnose-Treffer nur an der einen Zelle behoben, die dem gemeldeten Symptom am nächsten lag (`tbl00` in `Sub_POS` selbst) — die vier nachgelagerten SubBand-Zellen aus Eintrag 15 sowie drei weitere Geschwisterzellen in derselben `Sub_POS`-Zeile (`tc_CellPos`, `tc_CellAnzahl`, `tc_EPreis`, `tc_GPreis`) blieben unangetastet, obwohl sie strukturell zum selben Muster gehören. Erst ein erneuter, gezielter struktureller Diff gegen die Referenz (ausgelöst durch den Screenshot-Hinweis) deckte auf, dass insgesamt acht statt einer Zelle betroffen waren.

**Was man daraus lernt:** Eine Katalog-Formulierung im Plural bzw. mit „alle betroffenen ..." ist ein Vollständigkeits-Signal, kein Stilmittel — sie bedeutet, dass am ersten gefundenen Treffer nicht aufgehört werden darf, sondern **alle** strukturell analogen Kandidaten (hier: alle Geschwisterzellen derselben Tabellenzeile UND alle direkt nachfolgenden, bedingt gedruckten SubBands) systematisch aufgezählt und geprüft werden müssen, auch wenn nur einer davon das ursprünglich gemeldete Symptom ausgelöst hat. Das Muster ähnelt Eintrag 34 (unvollständiger Katalog-Scope), betrifft hier aber nicht das Überspringen ganzer Kataloge-Einträge, sondern das vorzeitige Abbrechen der Aufzählung INNERHALB eines einzelnen, bereits erkannten Musters.

**Wie man es künftig vermeidet:** Neue, dauerhafte Regel „Vollständigkeits-Heuristik" (siehe SKILL.md Schritt 2, seit v1.11.0): Enthält ein zutreffender Katalog-Eintrag eine Plural- oder Alle-Formulierung („nachfolgende(n) SubBand(s)", „alle betroffenen Zellen/Controls", „jede/r ..."), werden vor Anwendung des Fixes ausdrücklich ALLE strukturell analogen Kandidaten aufgezählt (z. B. alle Geschwister-Zellen derselben Zeile, alle direkt nachfolgenden gleichartigen SubBands) und einzeln gegen die Referenz geprüft — nicht nur der erste, der zum gemeldeten Symptom passt. Diese Aufzählung wird im Befund an den Nutzer (Schritt 3) explizit als Liste sichtbar gemacht, damit eine unvollständige Behebung schon vor der Auslieferung auffällt, nicht erst beim nächsten Kunden-Testdruck.

---

## 37. Ein neu angelegtes Sammlungs-Element mit ungültigem Platzhalter-Tag-Namen (`<ItemPLACEHOLDER>` statt `<ItemN>`) ergibt gültiges XML, wird aber von DevExpress beim Laden stillschweigend verworfen — und bei einem späteren Rebuild derselben Sammlung endgültig gelöscht

**Wann aufgefallen:** 09.09.2026, Report `dxAio_template` (M001), Nachbesserung 2 — Auslöser war ein Designer-Screenshot des Kunden („Band hat noch nicht die richtige Größe", `Sub_POS_Ueberschriften_Anzeige`).

**Was passiert ist:** Ein früherer Fix-Lauf hatte drei neue `<LocalizationItems>`-Einträge angelegt (Band-Höhe `Sub_POS_Ueberschriften_Anzeige` sowie Position/Größe des Labels `lblDocGesamtGesPreis`), dabei aber versehentlich den Tag-Namen als `<ItemPLACEHOLDER Ref="..." .../>` statt fortlaufend als `<ItemN ...>` geschrieben. Die anschließende Umnummerierungs-Routine (`re.sub(r'^<Item\d+', ...)`) verlangt Ziffern direkt nach „Item" — „PLACEHOLDER" enthält keine, die Regex griff also nicht, und der ungültige Tag-Name blieb unverändert im XML stehen. Das Ergebnis war weiterhin wohlgeformtes XML (kein Parse-Fehler, `C02`/`C03` grün) — DevExpress selbst erkennt ein `<LocalizationItems>`-Kind aber offenbar nur, wenn dessen Tag-Name exakt dem `ItemN`-Muster entspricht, und verwirft ein abweichend benanntes Kind beim Laden vollständig und ohne jede Fehlermeldung. Bestätigt durch eine vom Kunden im DevExpress-Designer neu gespeicherte Datei: dort fehlten alle drei Einträge tatsächlich vollständig. Ein nachfolgender Lauf, der die komplette `LocalizationItems`-Sammlung anhand des Musters `<Item\d+ ...>` neu einliest und durchnummeriert, hat die schon ungültigen Einträge dadurch zusätzlich endgültig aus der Datei entfernt, statt sie nur unwirksam zu belassen — aus einem unsichtbaren, aber im Rohtext noch vorhandenen Fehler wurde damit echter, nicht mehr rekonstruierbarer Datenverlust.

**Was man daraus lernt:** Ein Platzhalter-Tag-Name, der offensichtlich nach einem Platzhalter aussieht (`ItemPLACEHOLDER`, `ItemX`, `ItemNEU` o. Ä.), ist trotzdem ein Sammlungs-Element mit ungültigem Namen, solange er nicht exakt `Item\d+` entspricht — und keiner der bisherigen Validierungs-Checks (`C02` Wohlgeformtheit, `C03` Tag-Paarigkeit, `C04` ItemN-Lücken-Check) fängt das ab, weil `C04` nur bereits auf `Item\d+` passende Tag-Namen zählt und ein komplett anders benanntes Kind dadurch für den Check unsichtbar bleibt. Der Fehler ist damit strukturell identisch zur Klasse „ergibt gültiges XML, wird aber von DevExpress stillschweigend anders behandelt als erwartet" (vgl. Einträge 13, 21, 30) — nur diesmal mit vollständigem Datenverlust statt nur falschem Layout.

**Wie man es künftig vermeidet:** Neuer automatisierter Check `C22` in `scripts/validate_repx.py` (seit v1.11.0): geht strukturell (tiefenbewusst, per Tag-Stack) durch alle bekannten Sammlungs-Eltern-Tags (`Controls`, `Rows`, `Cells`, `LocalizationItems`, `ExpressionBindings`, `Bands`, `Parameters`, `ParameterPanelLayoutItems`, `StyleSheet` — empirisch gegen die Referenzdatei bestätigt, siehe Check-Kommentar im Skript) und meldet ein `FAIL`, sobald ein direktes Kind-Element vorkommt, dessen Tag-Name nicht exakt `Item\d+` entspricht. Zusätzlich, unabhängig vom automatisierten Check: Beim Anlegen NEUER Sammlungs-Einträge (nicht nur beim Umnummerieren bestehender) wird der Platzhalter-Name von Anfang an so gewählt, dass er bereits dem `ItemN`-Muster entspricht (z. B. `<Item0 Ref="0" .../>` statt `<ItemPLACEHOLDER ...>`), und nach jedem Rebuild derselben Sammlung wird aktiv gegengeprüft (Anzahl Einträge vorher/nachher, keine verbliebenen `Item0`/Platzhalter-Reste im Ergebnis).

---

## 38. `validate_repx.py` Check `C07` meldet bei isoliertem Aufruf ohne `--baseline` Falsch-Positive für Methoden, die ausschließlich in einem Subreport-eigenen `ScriptsSource`-Block definiert sind

**Wann aufgefallen:** 10.09.2026, Report `dxAio_template` (M001), Bugfix-Lauf — aufgefallen beim vorgeschriebenen Selbst-Audit der Referenzdatei mit `validate_repx.py` ohne `--baseline` (siehe SKILL.md Schritt 6a), unabhängig zweimal reproduziert (Builder-Rolle und, davon unabhängig, ein frischer Reviewer-Subagent in der Zwei-Rollen-QS-Schleife, Schritt 6b).

**Was passiert ist:** `python3 validate_repx.py referenz_dxAio_template.repx` (ohne `--baseline`) meldete `C07 FAIL` mit fünf angeblich fehlenden Methoden (`GroupHeader1_BeforePrint`, `ReportFooter_BeforePrint`, `SubBand_BelegRabatt_BeforePrint`, `SubBand_BelegZuschlag_BeforePrint`, `SubReportPartialInvoice_BeforePrint`). Eine gezielte Textsuche im Rohdokument zeigte: Alle fünf Methoden existieren tatsächlich, korrekt definiert und verdrahtet — aber nicht im Haupt-`ScriptsSource`-Block, sondern jeweils in einem der Subreport-eigenen `ScriptsSource`-Blöcke (jeder Subreport trägt seinen eigenen, unabhängigen Skript-Geltungsbereich, siehe `repx-technical-notes.md`). Ursache im Check selbst: `C07` sammelt die verdrahteten Methodennamen (`wired`) aus ALLEN `<Scripts>`-Elementen der GESAMTEN Datei, aber die definierten Methoden (`defined`) nur aus dem HAUPT-Skript — jede Methode, die nur in einem Subreport-Skript steht, aber im selben Subreport (oder, wie hier durch Namensgleichheit, zufällig auch anderswo) verdrahtet ist, erscheint dadurch fälschlich als „fehlend".

**Was man daraus lernt:** Der Check ist unvollständig auf genau dieselbe Art wie die bereits in Eintrag 10 dokumentierte Klasse „nicht sauber gescopte Paritätsprüfung" — nur diesmal nicht bei `<Scripts>`-Elementen vs. Freitext, sondern bei Haupt-Skript vs. Subreport-Skripten. Das Symptom trat unabhängig zweimal auf derselben, nachweislich korrekten Referenzdatei auf (kein Diagnosefehler eines einzelnen Laufs), betrifft aber ausdrücklich nur den isolierten Selbst-Audit-Modus (ohne `--baseline`) auf Dateien mit Subreports — der reguläre `--baseline`-Vergleichslauf auf der bearbeiteten Zieldatei zeigte in beiden unabhängigen Läufen `C07 OK`, war also nicht betroffen.

**Wie man es künftig vermeidet:** Bei einem `C07 FAIL` im Selbst-Audit-Modus (ohne `--baseline`) NICHT vorschnell von einem echten Datenfehler in der Referenzdatei ausgehen, sondern zuerst manuell per Grep prüfen, ob die „fehlende" Methode in einem Subreport-eigenen `ScriptsSource`-Block existiert — wenn ja, ist es dieser bekannte Validator-Fehlalarm, kein Report-Fehler. `scripts/validate_repx.py`s `C07`-Implementierung ist noch NICHT gefixt (offener technischer Punkt, kein Fix in diesem Lauf vorgenommen, um den Skript-Umfang dieses Laufs nicht zu sprengen) — eine korrekte Behebung müsste `defined`/`wired` jeweils PRO `ScriptsSource`-Block (Haupt- und jeder Subreport-Block separat) sammeln und vergleichen, statt global über die ganze Datei zu mischen, analog zur bereits für Skript-Scoping-Zwecke in `repx-technical-notes.md` beschriebenen Regel.

---

## 39. Muster (i) Punkt 4 wurde nur auf die neu angelegten Helper-Controls angewendet, nicht auf bereits vorher vorhandene Geschwister-Controls im selben Band

**Wann aufgefallen:** 10.09.2026, Report `dxAio_template` (M001), Nachbesserung nach Rückspielen der Runde-3b-Datei — Auslöser war eine Testdruck-Rückmeldung des Kunden, der den Fix bereits selbst in Visual Studio nachgezogen hatte, bevor er gemeldet wurde.

**Was passiert ist:** Der Band-Split aus Muster (i) (`DXJ0001.F`) war in einer früheren Runde auf das Band `Sub_POS_Ueberschriften_Folgeseiten` angewendet worden. Punkt 4 des Fixes verlangt, dass die Höhe **aller** betroffenen verschachtelten Controls auf den erzwungenen Minimalwert (effektiv 5) abgesenkt wird. Tatsächlich behandelt wurde dabei nur `lblDocGesamtGesPreis` — das im selben Lauf neu angelegte Helper-Label (siehe Eintrag 37, dasselbe Element). Zwei bereits **vorher** im selben Band vorhandene Geschwister-Controls blieben unangetastet: `xrLabel45` (eigener `<LocalizationItems>`-Eintrag `SizeF="100,90"`) und `lblCarryHelperOben` (eigener `<LocalizationItems>`-Eintrag `SizeF="160,40"`). Beide sind reine, unsichtbare Berechnungs-/Kontroll-Helper (`ForeColor="Transparent"` bzw. `"Black"` mit Kommentar „Helper-Label für Unterdrückung vom Übertrag") — funktional exakt die Art Control, die Punkt 4 meint —, wurden aber schlicht nicht mitgeprüft, weil der Fokus auf den im selben Arbeitsschritt neu erzeugten Elementen lag. Ergebnis: sichtbarer zusätzlicher Abstand beim Kunden-Testdruck, den der Kunde vor der Rückmeldung bereits selbst in Visual Studio auf Höhe 5 korrigiert hatte.

**Was man daraus lernt:** Dieselbe Grundlehre wie Eintrag 36 (Vollständigkeits-Heuristik), aber auf einer noch engeren Ebene: Dort ging es um mehrere strukturell *gleichartige* Fundstellen desselben Katalog-Punkts (Geschwisterzellen einer Tabellenzeile, nachfolgende SubBands). Hier reicht die Formulierung „alle betroffenen verschachtelten Controls" sogar über die im selben Fix-Schritt neu angelegten Elemente hinaus auf bereits **vorher bestehende** Controls im selben Band — ein Fix-Lauf, der Muster (i) anwendet, prüft leicht nur das, was er selbst gerade erzeugt hat, und übersieht dabei, dass ein Band schon vor dem Split weitere, unabhängig persistierte Höhen-Altlasten enthalten kann. Der Befund ist im reinen Skript-Diff so gut wie unsichtbar, weil beide übersehenen Controls über den `<Localization>`-Block persistiert sind (vgl. Eintrag 8).

**Wie man es künftig vermeidet:** `fix-catalog.md` Muster (i) Punkt 4 um eine verbindliche Ergänzung erweitert (seit v1.12.0): Vor Abschluss von Punkt 4 wird die vollständige `<Controls>`-Sammlung des betroffenen Anzeige-Bands durchgegangen — nicht nur die in Punkt 1 neu erzeugten Elemente — und für jedes Kind-Control einzeln die wirksame Höhe (direktes Attribut UND `<Localization>`-Eintrag) geprüft und als sichtbare Liste im Befund an den Nutzer aufgeführt. Das ist ein konkreter Unterfall der bereits bestehenden Vollständigkeits-Heuristik (SKILL.md Schritt 2, seit v1.11.0), hier ausdrücklich auf „bereits vorher vorhandene Geschwister-Controls im selben Band" zugespitzt, weil genau dieser Fall in der Praxis übersehen wurde. Neues Eval-Szenario `09-vorbestehende-geschwister-controls.json`.
