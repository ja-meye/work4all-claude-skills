...
## 2026-09-09 — Der EndUserDesigner-Rundlauf (VS-Projekt ⇄ .repx ⇄ Kunde/EUD) ist eine eigenständige Fehlerquelle, unabhängig von der DevExpress-Versionsfrage

**Kontext:** Interne Diskussion, ob die abweichende DevExpress-Version zwischen EndUserDesigner (v25.2, Laufzeitversion 25.2.9) und den VS-Report-Projekten (durchgängig 25.1.3) zu Problemen führen kann. Anlass war der bestehende Arbeitsablauf: Kunden bearbeiten ausgelieferte .repx-Dateien selbst im EndUserDesigner; gelegentlich werden diese Dateien zur Kontrolle zurückgespielt und ins VS-Ursprungsprojekt reimportiert (z. B. für Unterberichte); auch intern wird der EUD für Kleinanpassungen genutzt, das VS-Projekt bleibt aber immer Single Source of Truth. Wiederholt aufgetreten, auch beim Austausch von .repx-Dateien in dieser Session: fehlende Parameter-Namen, nicht mehr erkannte Subreports, Kompilierfehler im VS-Projekt nach dem Reimport.

**Befund:** Recherche in DevExpress-Doku und Support-Center zeigt zwei getrennte Ursachen, die sich in der Praxis überlagern:

1. Zur eigentlichen Minor-Versionsfrage (25.1 vs. 25.2) macht die DevExpress-Doku keine explizite Aussage — nur für Web-Reporting ist dokumentiert, dass Client-Script-Version und Server-Library-Version übereinstimmen müssen. Eine .repx ist XML und wird beim Öffnen vom Designer automatisch auf dessen eigene Version anhoben (dafür existiert der DevExpress-Converter); ein reiner Minor-Sprung ist für sich genommen ein eher kleines Risiko — aber eine Einbahnstraße: einmal mit dem neueren Designer gespeichert, lässt sich die Datei nicht mehr zuverlässig mit älterem Tooling öffnen.
2. Die deutlich größere, versionsunabhängige Fehlerquelle ist strukturell: Eine .repx-Datei ist niemals ein vollständiges Abbild eines VS-Projekt-Reports. Was im Code-Behind (`.Designer.cs`) lebt — programmatisch gesetzte Parameter, SQL-Query-Parameter-Bindungen (`?ArgXYZ`), per Code verknüpfte Subreports (`XRSubreport.ReportSource = ...`) — sieht ein standalone geöffneter EUD nicht oder nur indirekt. Beim Neuserialisieren gehen solche Bindungen verloren oder werden umbenannt (z. B. Auto-Namen wie `parameter1`). Das ist bei DevExpress dokumentiertes, wiederkehrendes Verhalten (Support-Center-Tickets u. a. „Save Reports as REPX - Parameters disappear", „Losing Datasource After importing a repx-File", „Report scripts are lost on importing a report from an REPX file"), kein Einzelfall bei uns. Der **Reimport ins VS-Projekt** ist der eigentliche Risikomoment, nicht das reine Öffnen im EUD.

**Konsequenz:** Bei jeder extern (Kunde oder EUD) bearbeiteten .repx vor dem Reimport ins VS-Projekt gezielt auf drei Punkte prüfen, nicht nur auf „kompiliert": (a) Parameter-Liste vollständig und korrekt benannt, (b) Subreport-Bindungen intakt, (c) Datenquellen-/Query-Parameter-Bindung unverändert. Der EUD sollte nur für rein layoutbezogene Änderungen ohne Eingriff in Parameter/Datenquelle/Subreports freigegeben werden — sobald eine dieser drei Kategorien betroffen ist, gehört die Änderung ins VS-Projekt. Die DevExpress-Versionsfrage (25.1.3 vs. 25.2, geplanter Umstieg auf 26.x nach Release) ist davon unabhängig zu entscheiden und bleibt zum Zeitpunkt dieses Eintrags offen.
